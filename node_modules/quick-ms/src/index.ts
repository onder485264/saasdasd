...
import { timeObject, accurateTimeObject, readableOptions } from './types';
 ...
 * @access public
 */
function getReadableTime(ms: number, opt: readableOptions = { compact: false, showEmpty: false, showMS: false }): string {
    ...
    if (opt.compact) {
        ...
        else if(opt.showEmpty) reply.push('0yrs');
        if (t.months) reply.push(`${t.months}mo`);
        else if(opt.showEmpty) reply.push('0mo');
        if (t.days) reply.push(`${t.days}d`);
        else if(opt.showEmpty) reply.push('0d');
        if (t.hours) reply.push(`${t.hours}hrs`);
        else if(opt.showEmpty) reply.push('0hrs');
        if (t.minutes) reply.push(`${t.minutes}min`);
        else if(opt.showEmpty) reply.push('0min');
        if (t.seconds) reply.push(`${t.seconds}sec`);
        else if(opt.showEmpty) reply.push('0sec');
        if (t.milliseconds && opt.showMS) reply.push(`${t.milliseconds}ms`);
        else if(opt.showEmpty && opt.showMS) reply.push('0ms');
    }
    else {
        if (t.years) reply.push(`${t.years} year${t.years > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 years');
        if (t.months) reply.push(`${t.months} month${t.months > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 months');
        if (t.days) reply.push(`${t.days} day${t.days > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 days');
        if (t.hours) reply.push(`${t.hours} hour${t.hours > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 hours');
        if (t.minutes) reply.push(`${t.minutes} minute${t.minutes > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 minutes');
        if (t.seconds) reply.push(`${t.seconds} second${t.seconds > 1 ? 's' : ''}`);
        else if(opt.showEmpty) reply.push('0 seconds');
        if (t.milliseconds && opt.showMS) reply.push(`${t.milliseconds} millisecond${t.milliseconds > 1 ? 's' : ''}`);
        else if(opt.showEmpty && opt.showMS) reply.push('0 milliseconds');
    }

    if (reply.length > 0) return reply.join(', ');
    else {
        if(opt.compact) return '0sec';
 ...
 * @access public
 */
function getAccurateTimeObject(ms: number): accurateTimeObject {
    if (!ms || typeof ms !== 'number' || !isFinite(ms)) throw new TypeError(`Final value is greater than Number can hold or you provided invalid argument.`);

    const result: accurateTimeObject = {
        centuries: 0,
        decades: 0,
        calendarYears: 0,
        months: 0,
...
export { getMilliseconds, getTimeObject, getReadableTime, getAccurateTimeObject };